...
import { ListAproveInvite, ListProjectWithStatusSearch } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
...
export default function SendApproveLetter({searchTerm}: Props) {
  ...
  const listProject = async (numberOfPage: number, searchName: string) => {
    let res;
        if (searchName.trim() === "") {
            res = await ListAproveInvite(numberOfPage * 5, 5);
        } else {
            res = await ListProjectWithStatusSearch(numberOfPage * 5, 5, 3, searchName);
        }
  ...
  const handleLinkClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    navigate(`/contractor/waitsendparticipant?key=${keyValue}`);
  };
      ...
      <div className="flex flex-col m-3 px-10 py-2 rounded-2xl">
        ...
        <div className="rounded-lg border">
          <table className="w-full rounded-lg table-fixed">
            <thead className="text-white bg-[#0083FF] h-14 rounded-lg">
              <tr className="rounded-lg text-xl">
                ...
                <th>สถานะ</th>
                <th className="rounded-tr-lg">ส่งขออนุมัติ</th>
              </tr>
            </thead>
