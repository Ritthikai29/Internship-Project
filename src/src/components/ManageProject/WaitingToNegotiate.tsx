...
import { listWaitNegotiate, ListProjectWithStatusSearch } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
...
export default function SendInviteLatter() {
    ...
    const listProject = async (numberOfPage: number, searchName: string) => {
        let res;
        if (searchName.trim() === "") {
            res = await listWaitNegotiate(numberOfPage * 5, 5);
        } else {
            res = await ListProjectWithStatusSearch(numberOfPage * 5, 5, 7, searchName);
        }
