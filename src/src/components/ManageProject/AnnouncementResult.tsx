...
import { listProjectAnnouncementResult, ListProjectWithStatusSearch } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
...
export default function SendInviteLatter() {
  ...
  const listProject = async (numberOfPage: number, searchName: string) => {
    let res;
        if (searchName.trim() === "") {
            res = await listProjectAnnouncementResult(numberOfPage * 5, 5);
        } else {
            res = await ListProjectWithStatusSearch(numberOfPage * 5, 5, 8, searchName);
        }
  ...
  const navigate = useNavigate();
    ...
    const handleClickopen = (key: any) => {
        navigate(`/contractor/AnnouncementResultComponent/${key}`);
    }
            ...
            {Array.isArray(listProjects) &&
              listProjects.map((listProject) => (
                <tbody className="border-b-lg rounded-xl h-14">
                  <tr key={listProject.id} className="border-b text-xl"
                  style={{ verticalAlign: "top" }}>
                    ...
                    <td className="text-left text-lg py-3 hover:text-violet-400" style={{ cursor: "pointer" }} onClick={()=>showFullaffiliationNamePopup(`${listProject.division_name} / ${listProject.department_name} / ${listProject.SECTION} / ${listProject.SUBSECTION}`)}>{(`${listProject.division_name} / ${listProject.department_name} / ${listProject.SECTION} / ${listProject.SUBSECTION}`.length) > 15 ? `${(`${listProject.division_name} / ${listProject.department_name} / ${listProject.SECTION} / ${listProject.SUBSECTION}`).substring(0, 12)}...` : `${listProject.division_name} / ${listProject.department_name} / ${listProject.SECTION} / ${listProject.SUBSECTION}`}</td>
                    ...
                    <td className="text-center py-4">{listProject.status_name}</td>
                    <td className="text-center py-4">
                      <button
                        className="p-1.5 border rounded-3xl px-10 bg-white text-[#F22738] text-lg drop-shadow-md"
                        onClick={() => {
                          handleClickopen(listProject.key)
                      }}>
                         คลิก

                         {/* onClick={() => {
                          handleClick(
                            String(listProject.id) || "",
