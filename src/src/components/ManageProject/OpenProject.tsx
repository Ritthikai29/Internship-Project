...
import { ListOpenProject, ListProjectWithStatusSearch } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
...
export default function SendInviteLatter() {
  ...
  const [listProjects, setListProjects] = useState<ProjectWaitInterface[]>([])
    ...
    const listProject = async (numberOfPage: number, searchName: string) => {
        let res;
        if (searchName.trim() === "") {
            res = await ListOpenProject(numberOfPage * 5, 5);
        } else {
            res = await ListProjectWithStatusSearch(numberOfPage * 5, 5, 2, searchName);
        }
