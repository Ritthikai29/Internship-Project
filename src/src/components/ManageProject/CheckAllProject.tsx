...
import { ListAllProject, ListProjectWithStatusSearch } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
import { datetimeFormatter, showFileOnClick } from "../../services/utilitity";
import ReactPaginate from "react-paginate";
import { ListWaitProject } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
import Swal from "sweetalert2";

export default function CheckAllProject() {
  ...
  const listProject = async (numberOfPage: number, searchName: string) => {
        let res;
        if (searchName.trim() === "") {
            res = await ListAllProject(numberOfPage * 5, 5);
        } else {
            res = await ListProjectWithStatusSearch(numberOfPage * 5, 5, 9, searchName);
        }
              ...
              listProjects.map((listProject) => (
                <tbody className="border-b-lg rounded-xl h-14">
                  ...
                  >
                    ...
                    <td className="text-center py-4 pr-1">
                      {listProject.status_name}
                    </td>
                    {/* <td className="text-center">

                                        <button className="p-1.5 border rounded-3xl px-10 bg-white text-[#F22738] text-lg drop-shadow-md"
                                            onClick={() => {
                                                // showFileOnClick(listProject?. || "")
                                            }}>
                                            คลิก'
                                        </button>
                                    </td> */}
                ...
                <div className="my-12">
                    <p>Showing {currentPage + 1} to {totalPages} of {totalProject} entries</p>
                    <div className="flex justify-end">
                        <ReactPaginate
