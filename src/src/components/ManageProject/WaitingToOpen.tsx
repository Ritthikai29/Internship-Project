...
import { ProjectWaitInterface, ListTopicValidateProject } from "../../models/Project/IListWaitProject";
import { listWaitOpenPrice, ListProjectWithStatusSearch } from "../../services/ProjectWithStatusService/ProjectWithStatusService";
import ReactPaginate from "react-paginate";
import { apiUrl, datetimeFormatter } from "../../services/utilitity";
...
export default function SendInviteLatter() {
    ...
    const listProject = async (numberOfPage: number, searchName: string) => {
        let res;
        if (searchName.trim() === "") {
            res = await listWaitOpenPrice(numberOfPage * 5, 5);
        } else {
            res = await ListProjectWithStatusSearch(numberOfPage * 5, 5, 6, searchName);
        }
          ...
          <table className="w-full rounded-lg table-fixed">
            <thead className="text-white bg-[#2B2A2A] h-14 rounded-lg">
              <tr className="rounded-lg text-xl">
                ...
                <th className="rounded-tr-lg">สถานะ</th>
                ...
                <tbody className="border-b-lg rounded-xl h-14">
                  ...
                  >
                    ...
                    {/* <td className="text-center">
                                        ...
                                        <Link to={`${apiUrl}`} className="p-1.5 border rounded-3xl px-10 bg-white text-[#F22738] text-lg drop-shadow-md">
                                            คลิก
                                        </Link>
                                    </td> */}
                  </tr>
