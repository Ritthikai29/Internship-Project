...
export default function CommitteePasscode() {
  interface waitOpenBiddingInterface {
    open_id: string;
    passcode: string;
  }
  ...
  const [passcode, setPasscode] = useState<Partial<waitOpenBiddingInterface>>({
    open_id: openId || "",
  });
  ...
  const handleSubmit = async () => {
    ...
    if (res.status !== 200) {
      ...
      navigate(
        `/committee/projectwaittoopen/waitforothercommittee?open_id=${openId}`
      );
    }
  };
        ...
        <div className="bg-white drop-shadow-lg rounded-xl border w-full my-5 mr-5">
          ...
          <div className="px-32 py-14 flex flex-col gap-8">
            <div className="flex flex-row ">
              <label className="text-[#2B3467] text-2xl basis-1/2">
                <b>สถานที่เปิดซอง :</b>{" "}
                <LimitStringWithUrl string={consult?.open_place || ""} maxChars={35}/>
              </label>
              <label className="text-[#2B3467] text-2xl">
        ...
        <div className="bg-white drop-shadow-lg rounded-xl border w-full my-5 mr-5">
          <div className="px-32 py-10 ">
            <p className="text-2xl text-[#2B3467] font-bold ">
              รหัส OTP สำหรับการเปิดซองจะถูกส่งไปยังอีเมลของท่าน
            </p>
            ...
            <div className="grid grid-cols-3 ">
              <button
                ...
                onClick={() => navigate("/committee/projectwaittoopen")}
              >
