...
export default function CAComment() {
                ...
                <table className="w-full drop-shadow-lg rounded-lg table-fixed">
                    ...
                    {comments
                        ...
                        .map((item, index) => (                        
                            ...
                            >
                            <td className="py-4 text-left pl-10">
                                {item.nametitle_t} {item.firstname_t} {item.lastname_t}
                            </td>
                            <td className="py-4">{item.role_name_t}</td>

                            <td className="py-4">
                                {item.status_comment === "success" ? (
                                <span className="py-4 pl-2 text-[#2FAC10]">
                                    {item.topic_comment}
                                </span>
                                ) : item.status_comment === "unsuccess" ? (
                                <span className="py-4 pl-2 text-[#ec7332]">
                                    {item.topic_comment}
                                </span>
                                ) : (
                                <span className="py-4 pl-2 text-[#fc2f2f]">
                                    {item.topic_comment}
                                </span>
                                )}
                            </td>

                            <td className="py-4">{item.detail_comment ? item.detail_comment : "-"}</td>

                            <td className="py-4">{dateWithTimeFormatter(item.submit_datetime)} น.</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
