...
import { GetVerifyProject, GetVerifyReason, VerifySubmit , GetUnitOfProject } from "../../../services/BudgetService/VerifyService";
...
export default function VerifyComponent(
    ...
    const getVerifyProject = async () => {
        const res = await GetVerifyProject(projectKey, null);
        setProject(res.data)
    }

    const getReason = async () => {
        const res = await GetVerifyReason();
        ...
        await MySwal.fire(
            {
                ...
                preConfirm: async () => {
                    const prepare = {
                        reason_id: Number(reasonRef.current?.value),
                        comment: commentRef.current?.value,
                        is_verify: false,
                        project_key: projectKey
                    }
                    // console.log(prepare)
                    const res = await VerifySubmit(prepare);
