    ...
    <table class="first-table">
        <tr>
            <td class="first-td">
                <br>
                <h1>
                    เรียน ' .$approver['employee_name']. '
                </h1>
                <h1>   
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;เพื่อพิจารณาอนุมัติผลการประกวดราคา 
                        โครงการ' . $project['name']  . ' เลขที่เอกสาร ' . $project['key']  . ' ซึ่งได้รับความเห็นชอบจากคณะกรรมการจ้างเหมา ปูนทุ่งสงแล้ว
                    ...
                    <tr>
                        <td class="second-td"  style="background-color: #1aa125;">
                            <a class="login-a" href="http://137.116.132.150/STSBidding/frontend/md/approve-bidding?key=' . $project['key']  . '" >
                                &nbsp;&nbsp;อนุมัติความเห็นชอบจากกรรมการ&nbsp;&nbsp;</a>
        ...
        <tr>
            <td class="first-td">
                ...
                <p>
                    ...
                    <p>                    
                            <span>หน่วยงาน :</span> ' . $project['afiliation']  . '
                    </p>
                    <p>
                            <span>วันที่เพิ่มโครงการ :</span> '. date('d/m/Y', strtotime($project['add_datetime'])) . '
                    </p>
...
function GenerateTable($vendors)
{
    ...
    foreach ($vendors as $index => $vendor) {
        if($vendor["result"] == "ชนะการประกวด"){
            if($vendor["vendor_type_name"] == "ใน List ทะเบียน"){
                $table .= '<tr class="data-row">
                        <td>' . ($index + 1) . '</td>
                        <td style=" color: #549743;">' . $vendor["result"] . '</td>
                        <td style=" color: #0083FF;">' . $vendor["vendor_type_name"] . '</td>
                        <td>' . $vendor["vendor_key"] . '</td>
                ...
                $table .= '<tr class="data-row">
                        <td>' . ($index + 1) . '</td>
                        <td style=" color: #549743;">' . $vendor["result"] . '</td>
                        <td style=" color: #FF0000;">' . $vendor["vendor_type_name"] . '</td>
                        <td>' . $vendor["vendor_key"] . '</td>
                        <td>' . $vendor["company_name"] . '</td>
                        <td>' . $vendor["manager_name"] . '</td>
            ...
            if($vendor["vendor_type_name"] == "ใน List ทะเบียน"){
                $table .= '<tr class="data-row">
                        <td>' . ($index + 1) . '</td>
                        <td style=" color: #FF0000;">' . $vendor["result"] . '</td>
                        <td style=" color: #0083FF;">' . $vendor["vendor_type_name"] . '</td>
                        <td>' . $vendor["vendor_key"] . '</td>
                        <td>' . $vendor["company_name"] . '</td>
                        <td>' . $vendor["manager_name"] . '</td>
