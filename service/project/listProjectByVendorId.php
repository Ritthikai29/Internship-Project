...
$userId = isset($token->vendorId) ? $token->vendorId : null;
if (!$userId) {
    $http->Unauthorize(
        [
...
if ($status) {
    $project = $projectService->listProjectsByStatusNameAndVendorId(
        userId: $userId,
        // offset: $offset,
        // limit: $limit,
        status: $status
    );
    $countProject = $projectService->countProjectsByStatusNameAndVendorId(
        $status,
        $userId
    );
}else{
    $project = $projectService->listProjectsByVendorId(
        // $offset, 
        // $limit, 
        $userId
    );
    $countProject = $projectService->countProjectsByVendorId(
        $userId
    );
}
