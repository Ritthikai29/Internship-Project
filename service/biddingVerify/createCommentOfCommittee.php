...
/**
 * insert a data to comment of the user 
 */
$prepareInserted = [
    "topic_id" => $topic["id"],
    "comment" => $comment,
...
if($directorOldComment){
    $commentupdate = $biddingVerifyService->updateCommentCommitteeDetail($prepareInserted);
    if(!$commentupdate){
        $biddingVerifyService->rollbackTransaction();
        $http->Forbidden(
            [
                "err" => "ไม่สามารถ update ข้อมูลคอมเม้นของกรรมการได้",
                "status" => 403
            ]
        );
    }
    ...
    $commentInserted = $biddingVerifyService->InsertedCommentCommittee($prepareInserted);
    if(!$commentInserted){
        $biddingVerifyService->rollbackTransaction();
        $http->Forbidden(
            [
                "err" => "ไม่สามารถเพิ่มข้อมูลคอมเม้นของกรรมการได้",
                "status" => 403
            ]
        );
    }
