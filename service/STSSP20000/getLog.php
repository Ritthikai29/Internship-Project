 ...
 * 
 */
$managers = $verifyService->listRefPriceManagers($project["id"]);
...
for ($i=0; $i < $cms; $i++) { 
     ...
     * @var array
     */
    $userStaff = $verifyService->getUserStaffById($managers[(int)$i]["user_staff_id"]);
     ...
     * @var array
     */
    $employee = $verifyService->getEmployeeById($userStaff["employee_id"]);
    // check user staff is found ? 
    if(!$employee){
        // if user is not found
        $http->NotFound(
 ...
 * @var array
 */
$logCalculates = $verifyService->listLogFromManagerListId($managers, $cms);
