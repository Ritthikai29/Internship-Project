...
/**
 * get a project in ready to approving to the project
 */
$project = $openBiddingService->getAllProjectReadyToConsult();

forEach($project as $index => $value){
    $project[$index]["totalVendor"] = ($openBiddingService->getTotalVendorByProjectId($value["id"]))["total"];
    $project[$index]["totalVendorRegistor"] = ($openBiddingService->getTotalVendorRegisterByProjectId($value["id"]))["total"];
}
