...
// Get Vendor info
$vendorRegisterInfo = $registerProjectService->getVendorRegisterInfoByVdIdAndPjId($vendorId,$project['id']);
if (!$vendorRegisterInfo) {
    $http->NotFound(
        [
            "err" => "not found vendorRegister",
            "status" => 404
        ]
    );
}

$vendorRegisterInfo["price"] = $enc->bidDecode($vendorRegisterInfo["price"] ) ;
