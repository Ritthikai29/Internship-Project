...
$vendorFile = $template->valFile(isset($_FILES["vendor_file"]) ? $_FILES["vendor_file"] : null, "ไฟล์ของVendor");
    ...
    $database_path = '/vendors/' . $vendorKey;
        ...
        $bookbankLocation = $upload_path . '/bookbank.' . $fileExt;
        $bookbankDatabase = $database_path . '/bookbank.' . $fileExt;

        $data=[
            "bookbank_uri"=> $bookbankDatabase,
            "vendor_key"=> $vendorKey            
        ];
        $vendorService->startTransaction();
        $fileSaved = $vendorService->updateBookbankFileVendor($data); 
        if (!$fileSaved) {
     ...
     */
        ...
        try {
            move_uploaded_file($vendorFile["TempName"], $bookbankLocation);
        } catch (Exception $e) {
            $vendorService->rollbackTransaction();
            $http->BadRequest([
                "err" => $e->getMessage(),
