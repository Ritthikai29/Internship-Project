...
$vendorFile = $template->valFile($_FILES["vendor_file"], "ไฟล์ของVendor");
    ...
    $database_path = '/vendors/' . $vendorKey;
        ...
        $VATLocation = $upload_path . '/VAT.' . $fileExt;
        $VATDatabase = $database_path . '/VAT.' . $fileExt;
        ...
        $fileSaved = $vendorService->updateVatFileVendor($data); 
        if (!$fileSaved) {
     ...
     */
        ...
        try {
            move_uploaded_file($vendorFile["TempName"], $VATLocation);
        } catch (Exception $e) {
            $vendorService->rollbackTransaction();
            $http->BadRequest([
                "err" => $e->getMessage(),
